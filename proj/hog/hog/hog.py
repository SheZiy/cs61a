...
def roll_dice(num_rolls, dice=six_sided):
    ...
    so_sad=False   #是否投出了1
    score=0 #单次投掷点数
    while num<=num_rolls:   #投掷骰子num_rolls次
        score=dice()
        if score==1:
            so_sad=True
        total+=score
...
def boar_brawl(player_score, opponent_score):    
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    player_digit=player_score%10    #玩家得分的个位数
    opponent_digit=0                #对手得分的十位数
    if  opponent_score//10:
        opponent_digit=opponent_score//10%10
    return max(3*abs(player_digit-opponent_digit),1)    #返回三倍差值和1中最大者
    # END PROBLEM 2
...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    result=0
    if num_rolls==0:
        return boar_brawl(player_score,opponent_score)  #选择不投掷骰子，按野猪大战结算点数
    else:
        return roll_dice(num_rolls,dice)                #选择投掷骰子
    # END PROBLEM 3
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # 使用 while 循环遍历所有的得分组合
    i = 0
    while i < goal:
        j = 0
        while j < goal:
            # 如果策略输出不同，立即返回 False
            if strategy(i, j) != initial_roll:
                return False
            j += 1
        i += 1
    return True
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):   
    ...
    score=0
    while num_rolls<=10 :
         #roll_dice只考虑了so_sad规则，没有考虑boar_brawl和sus_fuss规则
...
def final_strategy(score, opponent_score):
    ...
    两个改进：
            1）检查是否可以通过更低风险的投掷骰子到达goal
            2）不使用threshold,只要不投掷骰子得分高于投6个骰子的平均数

    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    #测试不投掷骰子得分是否高于投6个骰子的平均得分
