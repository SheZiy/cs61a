...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    paragraphs_selected=[selected for selected in paragraphs if select(selected)]
    if k>= len(paragraphs_selected) :
        return ''
    else :
...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def about_compare(paragraph):#subject是包含一个个单词的列表
        #将paragraph处理成单词的列表
        processed_paragraph=split(remove_punctuation(lower(paragraph)))
        #检查paragraph中是否有单词与subject中的相同
        for word in processed_paragraph :
            for item in subject :
                if word==item:
                    return True
        return False
    return about_compare
    # END PROBLEM 2
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    while index<len(word_list) :
        cur_diff=diff_function(typed_word,word_list[index],limit)
...
def furry_fixes(typed, source, limit):
    ...
    # BEGIN PROBLEM 6
    """
    def num_change(typed,source,change) :
        if change>limit :
            return change
        if len(typed)>0 and len(source)>0 :
            if typed[0]==source[0] :
                typed=typed[1:]
                source=source[1:]
                return num_change(typed,source,change)
            else :
                typed=typed[1:]
                source=source[1:]
                return num_change(typed,source,change+1)
        else :
            return change+abs(len(typed)-len(source))
    return num_change(typed,source,0)
    """
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = []
    for time_point in tpp:
        i = 0
        time = []
        while i<len(time_point) - 1:
            t = time_point[i+1] - time_point[i]
            time+=[t]
            i+=1
        times += [time]
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for w in word_indices:
        fastest_time = min([times[p][w] for p in player_indices])
        for p in player_indices:
            if fastest_time == times[p][w]:
                fastest[p] += [words[w]]
                #fastest[p].append(words[w])
                break
    return fastest
    # END PROBLEM 10
