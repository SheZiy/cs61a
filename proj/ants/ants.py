...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        cur = 0
        while near_place.is_hive == False:
            if near_place.bees and cur >= self.lower_bound and cur <= self.upper_bound:
...
# The WallAnt class
class WallAnt(Ant):
    """A WallAnt does nothing each turn,but it has a large health value"""
...
# The HungryAnt Class
class HungryAnt(Ant):
    """HungryAnt will select a random Bee from its place to eat(deal damage to the Bee equal to the Bee's health).After eating a Bee,a HungryAnt must speed 3 turns chewng before being able to eat again.While the HungryAnt is chewing,it is not able to eat any Bees.
    """
...
class ContainerAnt(Ant):
    ...
    def store_ant(self, ant):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        assert self.ant_contained == None
...
class TankAnt(ContainerAnt):
    """TankAnt can do damage to all bees in its place each turn"""
...
class ScubaThrower(ThrowerAnt):
    "ScubaThrower is a type of Thrower who are waterproofed"
